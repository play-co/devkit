...
function pathGetter() {
    ...
    return function () {
        return path.join.apply(path, base.concat(Array.prototype.slice.call(
            arguments)));
    };
}
...
exports.paths = {
    ...
    timestepAddons: pathGetter('lib', 'timestep', 'src', 'platforms', 'native',
        'addons'),
    ...
    if (!seenException) {
        ...
        case 'EADDRINUSE':
            console.error(color.red("ERROR:"),
                "Port 9200 already in use, exiting. (Are you serving elsewhere?)"
            );
            break;
        default:
            console.error(color.red("Uncaught Exception:"), e.stack);
        }
        common.track("BasilCrash", {
            "code": e.code,
            "stack": e.stack
        });
        console.log("");
...
function runChildCommand(childCommand, prog, args, opts, cont) {
    ...
    var out = [];
    var err = [];
    var formatter = new exports.Formatter(opts.tag || prog, opts.silent, out,
        err);
    ...
    tool.on('close', function (code) {
        if (code !== 0 && !opts.silent) {
            if (code === -1 || code === 127) {
                formatter.error('Unable to spawn ' + prog +
                    '.  Please make sure that you have installed all of the prerequisites.'
                );
            } else {
...
exports.Formatter = Class(function () {
    ...
    this.format = function (str) {
        ...
        .replace(/\d*(^|\s|[^a-zA-Z0-9-])error(s|\(s\))?/gi, function (res) {
            return color.redBright(res);
        }).replace(/\d*(^|\s|[^a-zA-Z0-9-])warn(ing)?(s|\(s\))?/gi,
            function (res) {
                return color.yellowBright(res);
            }).replace('BUILD SUCCESSFUL', color.greenBright(
            'BUILD SUCCESSFUL')).replace('BUILD FAILED', color.redBright(
            'BUILD FAILED'))
        // fix new lines
        .replace(/\r?\n(?!$)/g, '\n' + this._prefix);
    }
    // call these for formatted logging from code
    this.debug = this.info = this.log = function () {
        console.error.apply(console, [this._prefix].concat(Array.prototype.map
            .call(arguments, this.format, this)));
    }
    this.warn = function () {
        console.error.apply(console, [this._prefix, color.yellowBright(
            '[warn] ')].concat(Array.prototype.map.call(arguments, this
            .format, this)));
    }
    this.error = function () {
        console.error.apply(console, [this._prefix, color.redBright(
            '[error] ')].concat(Array.prototype.map.call(arguments,
            this.format, this)));
    }
...
exports.Formatter.getPrefix = function (tag) {
    return color.cyanBright(('               [' + tag + ']   ').substr(-15 -
        Math.max(0, tag.length - 10)));
}
...
exports.getLocalIP = function (next) {
    ...
    for (var name in interfaces) {
        if (/e(n|th)\d+/.test(name) || isWindows) {
            for (var i = 0;
                var item; item = interfaces[name][i]; ++i) {
    ...
    try {
        ...
        var data = lines.map(function (line) {
            ...
            if (match) {
                ...
                if (details && details[1]) {
                    var filename = path.relative(exports.paths.root(),
                        match[2]);
        ...
        data.slice(0, n - 1).map(function (data, i) {
            if (typeof data == 'string') {
                ...
                out.push(new Array(i + 1).join(' ') + '└ ' + color.green(
                        data.file) + ':' + color.blueBright(data.line) +
                    ' ' + color.whiteBright(data.func) + color.yellowBright(
                        ' (' + data.details + ')'));
            }
        });
        ...
        if (lastLine) {
            ...
            out.push(indent + '└ ' + color.redBright(msgText + (msgLines.length >
                    1 ? '\n' + indent : '') + color.white(' at ') + color.green(
                    lastLine.file) + ':' + color.blueBright(lastLine.line) +
                ' ' + color.redBright(lastLine.func) + color.yellowBright(
                    ' (' + lastLine.details + ')')));
        }
    } catch (e) {
        out.push(e.stack);
