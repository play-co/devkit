...
exports.config.write = function (cb) {
	...
	if (cb) { this._onWrite.push(cb); }

	if (this._isWriting) {
		// if we're already writing, flag that we need to write again
		this._hasPendingWrite = true;
	} else {
		// clear the pending write flag
		this._hasPendingWrite = false;
		this._isWriting = true;
		fs.writeFile(CONFIG_PATH, JSON.stringify(_config, null, '\t'), 'utf8', bind(this, function (err) {
			this._isWriting = false;

			if (this._hasPendingWrite) {
				// a write was scheduled, so we can't fire the callbacks until we write again
				exports.config.write();
			} else {
				// fire any/all the callbacks
				var onWrite = this._onWrite;
				this._onWrite = [];
				onWrite.forEach(function (cb) { cb(err); });
			}
		}));
	}
}
