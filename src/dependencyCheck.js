        ...
        var f = ff(function () {
            ...
            if (exists) {
                ...
                var f2 = ff(function () {
                    ...
                    var fileVersion = link.match(
                        /tealeaf-build-tools-(.*?)\.jar/);
                    if (!fileVersion) {
                        logger.log("tealeaf-build-tools", version.toString(),
                            "is missing, updating...");
                        next();
...
function run() {
    var f = ff(function () {
        ...
        f(package);
        var buildToolsVersion = Version.parse(package['basil'][
            'tealeaf-build-tools'
        ]);
        ...
        var addons = package['basil'][
            fs.readFileSync('.git/config', 'utf8').indexOf('devkit-priv') == -
            1 ? 'addons' : 'addons-priv'
        ];
