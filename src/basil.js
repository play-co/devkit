...
function displayHeader() {
    var opts = [
        ...
        "camelCase or __score__?", "absolutely no jquery (only sizzle).",
        "First games. Then freeways. Then the USA.",
        "Building a time machine, one function at a time.",
        "Monkey see monkey do...", "Coffee and cheerios.",
        ...
        "Now we know. Noooow we know. We know now.", "Hate the game.",
        "Syms and shims", "Zoinks!", "Using DHTML technologies",
        "Life is squill.Pane", "Needs more ketchup."
    ].sort(function () {
    ...
    var colouredGCText = clc.whiteBright("{{") + clc.cyanBright(
        " Game Closure SDK ") + clc.whiteBright("}}");
    console.log(["============================================================",
        "                   " + colouredGCText + "                   ",
        "                                                            ".substr(
            0, (60 - versionLength) / 2) + version,
        "                                                            ".substr(
            0, (60 - opts[0].length) / 2) + opts[0],
        "------------------------------------------------------------", ""
    ].join('\n'));
}
...
function ensureLocalProject() {
    if (!fs.existsSync('./manifest.json')) {
        console.error(clc.red('ERROR: '),
            'Basil started in non-GC project folder, aborting.');
        process.exit(2);
    }
}
    ...
    common.child('java', ['-version'], {
        ...
        if (err) {
            console.log(clc.red('ERROR:'),
                'Java is required in order to run this basil command.');
            process.exit(2);
        }
        next();
    });
...
function printHelp() {
    console.log(["basil [command]", "  about", "  build {target}", "  serve",
        "  testapp", "  init {name}", "  register", "  unregister",
        "  clean-register", "  install {addon}", "  update", "",
...
function commandHelp(command) {
    switch (command) {
    case 'build':
        console.log(["Builds a packaged game for the platform you specify.",
            "  basil build [platform] [--no-compress] [--clean] [--debug]",
            "", "Example usage:",
            "  basil build native-android --no-compress", "", "Platforms:",
    ...
    case 'serve':
        console.log([
            ...
            "Example usage:", "  basil serve -p 8080", "", "Flags:",
        ...
        break;
    case 'testapp':
        console.log(["Launches a Test App on a connected mobile device.",
            "  basil testapp [target]", "", "Example usage:",
            "  basil testapp native-ios", "", "Further information:",
    ...
    case 'register':
        console.log([
            ...
            "  basil register [path(s)]", "", "Example usage:",
    ...
    case 'unregister':
        console.log([
            ...
            "  basil unregister [path(s)]", "", "Example usage:",
        ...
        break;
    case 'clean-register':
        console.log(["Removes all invalid projects from the basil registry.",
            "  basil clean-register", "", "Example usage:",
            "  basil clean-register", "", "Further information:",
    ...
    case 'init':
        console.log([
            ...
            "Example usage:", "  basil init ./projects/myNewProject", "",
            "Templates:", "  empty", "  stackview", "",
            "Further information:",
...
if (require.main === module) {
    var f = ff(function () {
        ...
        initBuild(f());
    }).success(main).error(function (err) {
