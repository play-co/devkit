...
function initBuild (cb) {
	require(common.paths.lib('timestep', 'build')).init(build, cb);
}
...
function main () {
	...
	switch (process.argv[2]) {
		...
		case 'update-android': //this might want to change to a shell script some day
			require('./update-android').updateAndroid();
			return;
		...
		case 'build':
			...
			var f = ff(this, function () {
				...
				build.exec(process.argv.slice(3), {
