...
function listFiles(base, exts, cb) {
    ...
    findit.find(base).on('file', function (file, stat) {
        ...
        if ((exts || []).indexOf(path.extname(file).substr(1)) >= 0) {
            return ret.emit('file', path.basename(file, path.extname(file)),
                file, stat);
        }
    }).on('end', function () {
        return ret.emit('end');
...
function listFilesSync(base, exts) {
    ...
    if (fs.existsSync(base)) {
        findit.sync(base, function (file, stat) {
            if (stat.isFile() && (exts || []).indexOf(path.extname(file).substr(
                1)) >= 0) {
                files.push({
...
function createUUId(a) {
    return a ? (a ^ Math.random() * 16 >> a / 4).toString(16) : ([1e7] + 1e3 +
        4e3 + 8e3 + 1e11).replace(/[018]/g, createUUId);
}
/**
 * Module API
 */
...
exports.isPackage = function (root) {
    return fs.existsSync(root) && fs.existsSync(path.join(root, 'manifest.json'));
};
/*
 * Package Class
 */
var GCPackage = Class(function () {
    ...
    this.initPaths = function (root) {
        if (!exports.isPackage(root)) {
            return new Error(
                'Invalid game archive (expecting ./ and ./manifest.json)');
        }
    ...
    this.readManifest = function (cb) {
        ...
        fs.readFile(this.paths.manifest, function (err, contents) {
            ...
            if (!this.manifest.appId || (typeof this.manifest.appId !==
                'string') || (this.manifest.appId.length < 1)) {
                this.manifest.appId = createUUId();
                fs.writeFileSync(this.paths.manifest, serializeConfig(
                    this.manifest));
            }
    ...
    this.populateManifest = function (opts, cb) {
        ...
        this.saveManifest(augment({
            "appID": createUUId(),
            "shortName": (typeof opts.shortName === 'undefined') ?
                "" : opts.shortName,
            "title": (typeof opts.title === 'undefined') ? "" : opts
                .title,
            "studio": {
    ...
    this.saveManifest = function (data, cb) {
        this.manifest = data;
        return fs.writeFile(path.join(this.paths.root, 'manifest.json'),
            serializeConfig(data), cb);
    };
    this.saveManifestSync = function (data) {
        this.manifest = data;
        fs.writeFileSync(path.join(this.paths.root, 'manifest.json'),
    ...
    this.listIcons = function (cb) {
        return listFiles(this.paths.icons, ['png', 'jpg', 'jpeg', 'bmp',
            'gif'
        ], cb);
    };
    this.listImages = function (cb) {
        return listFiles(this.paths.resources, ['png', 'jpg', 'jpeg', 'bmp',
            'gif'
        ], cb);
    };
    this.listImagesSync = function () {
        return listFilesSync(this.paths.resources, ['png', 'jpg', 'jpeg',
            'bmp', 'gif'
        ]);
    };
    ...
    this.getTranslation = function (code, cb) {
        try {
            return fs.readFile(path.join(this.paths.lang, code + ".json"),
                function (err, data) {
    ...
    this.getTranslationSync = function (code) {
        try {
            var data = fs.readFileSync(path.join(this.paths.lang, code +
                ".json"));
    ...
    this.saveTranslation = function (code, json, cb) {
        ...
        return fs.writeFile(path.join(this.paths.lang, code + ".json"),
            serializeConfig(json), cb);
    };
    this.saveTranslationSync = function (code, json) {
        wrench.mkdirSyncRecursive(this.paths.lang, 0777);
        fs.writeFileSync(path.join(this.paths.lang, code + ".json"),
