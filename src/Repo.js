...
exports.create = Class(function () {
    this.init = function (name, def) {
        ...
        this.git = git.createClient(this.location, {
            silent: true
        });
    ...
    this.addRemote = function (name, uri, cb) {
        if (!name) {
            return this.log(
                "error: no name provided for remote in config.json");
        }
        if (!uri) {
            return this.log("error: no uri provided for remote", name,
                "in config.json");
        }
        var f = ff(this, function () {
            ...
            if (!foundName || !foundURI) {
                var next = f();
                this.git('remote', foundName ? 'set-url' : 'add', name,
                    uri, function (code, out, err) {
                        next(code ? {
                            code: code,
                            err: err
                        } : null);
    ...
    this.getCommitsSince = function (from, to, cb) {
        ...
        this.git('rev-list', '--left-right', '--oneline', from + '...' + to,
