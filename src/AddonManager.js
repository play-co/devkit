...
var Addon = Class(function () {
    ...
    this.getPath = function () {
        return common.paths.addons.apply(common.paths, [this._name].concat(
...
var AddonManager = Class(EventEmitter, function () {
    var Registry = Class(EventEmitter, function () {
        ...
        this.clone = function (cb) {
            ...
            if (!fs.existsSync(REGISTRY_PATH)) {
                ...
                logger.log("Cloning from", REGISTRY_URL,
        ...
        this.getVersion = function (addon, cb) {
            ...
            var f = ff(this, function () {
                ...
                fs.readFile(common.paths.lib('addon-registry/' +
                    addon + '.json'), 'utf-8', f());
            }, function (contents) {
                ...
                if (info.type == 'core') {
                    if (common.sdkVersion.channel == 'dev') {
                        ...
                        logger.log("Using version from SDK:",
                            common.sdkVersion.toString());
                        f(common.sdkVersion, true);
                    }
                } else {
                    var vers = info.version ? info.version : common
                        .paths.addons(addon).slice(-5) == "-priv" ?
                        "develop" : "master";
                    logger.log("Using version from registry:", vers
                        .toString());
                    f(vers, false);
                }
            }).cb(cb);
    ...
    this.getAddonDependencies = function (addon) {
        ...
        var configPaths = [path.join(addonPath, "ios", "config.json"), path
            .join(addonPath, "android", "config.json")
        ];
        var deps = [];
    ...
    this.install = function (addon, opts, cb) {
        ...
        var f = ff(this, function () {
            ...
            if (!addonExists) {
                logger.log(addon,
                    "not currently installed, installing now");
            ...
            if (doUpdate) {
                var properBranch = addonPath.slice(-5) == "-priv" ?
                    "develop" : "master";
                if (statusError || status.modified.length > 0) {
                    logger.error("Cannot update", addon,
                        "please stash or remove local changes first.");
                } else if (branchError || branch != properBranch) {
                    logger.error("Cannot update", addon,
                        "please checkout " + properBranch +
                        " branch first.");
                } else {
    ...
    this.clone = function (addon, version, opts, cb) {
        ...
        var addonPath = this.getPath(addon);
        var account; var repo; var target;
        ...
        var f = ff(this, function () {
            ...
            if (exists) {
                if (opts.force) {
                    ...
                    throw "Directory for " + addon +
                        " already exist. Please remove first.";
                }
            }
        }, function () {
            ...
            logger.error("Error downloading addon " + account + "/" +
                repo + ":\n", e);
        }).success(cb);
    };
    this.startupPlugins = function (next) {
        ...
        var f = ff(this, function () {
            ...
            if (links && links.length > 0) {
                for (var ii = 0;
                    var len = links.length; ii < len; ++ii) {
            ...
            if (targets && targets.length > 0) {
                ...
                for (var ii = 0;
                    var len = targets.length; ii < len; ++ii) {
            ...
            if (checks && checks.length > 0) {
                ...
                for (var ii = 0;
                    var len = checks.length; ii < len; ++ii) {
                    if (!checks[ii]) {
                        logger.log("Removing plugin JS symlink for",
                            links[ii], "since it no longer valid");
            ...
            logger.error("Failure validating plugin JS:", JSON.stringify(
                e));
        }).cb(next);
    }
    this.activatePluginJS = function (addon, next) {
        var pluginRoot = path.normalize(path.join(__dirname,
            "../sdk/plugins"));
        ...
        var jsPath = path.normalize(path.join(__dirname, "../addons", addon,
            "js"));
        var f = ff(this, function () {
            ...
            logger.error("Failure activating plugin JS for", addon +
                ":", JSON.stringify(e));
        }).cb(next);
    }
    /**
    ...
    this.activateVersion = function (addon, version, cb) {
        ...
        var f = ff(this, function () {
            ...
            if (code != 0) {
                logger.log("Could not find", version.toString(),
                    ". Using master");
            ...
            if (!fs.existsSync(currentPath)) {
                logger.warn(
                    "Couldn't find index.js! Your addon may break.",
                ...
                try {
                    ...
                    logger.error("Error loading [" + currentPath +
                        "] : init():");
                    console.error(e);
        ...
        var f = ff(this, function () {
            ...
            files.forEach(function (addonName) {
                if (!this._addons[addonName]) {
                    var currentPath = path.join(addonPath,
                        addonName, "index.js");
                    if (fs.existsSync(currentPath)) {
                        ...
                        try {
                            ...
                            // merge the paths in
                            this._paths = this._paths.concat(data.paths ||
                            ...
                            logger.error("Could not load [" +
                                addonName +
                                "] : No index file with a load method"
                            );
                            console.error(err.stack);
                        }
                    }
