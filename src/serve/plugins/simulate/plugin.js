...
function serveProject(project) {
    ...
    // Compiled targets.
    _app.use(root + 'debug/' + id, etag.static(path.join(project.paths.root,
        'build', 'debug'), {
        maxAge: 0
    }));
    _app.use(root + 'release/' + id, etag.static(path.join(project.paths.root,
        'build', 'release'), {
        maxAge: 0
    }));
    // var route = etag.static(path.join(project.paths.root, 'sdk', 'gc', 'debugging'));
    _app.get(new RegExp('^' + root + '.*?/sdk/gc/debugging/.*?'), function (req,
        res) {
        var pieces = parse(req.url).pathname.split('/sdk/gc/debugging/');
        res.sendfile(path.join(project.paths.root, 'sdk', 'gc', 'debugging',
            pieces[1]));
    });
}
...
exports.load = function (app, argv) {
    ...
    if (!nativeInspectorServer) {
        ...
        nativeInspectorServer.listen('csp', {
            host: 'localhost',
            port: '9225'
        });
        nativeInspectorServer.listen('tcp', {
            host: 'localhost',
            port: '9226'
        });
    }
    ...
    app.get('/simulate/:debug/:shortName/:target/', function (req, res, next) {
        common.getProjectList(function (projects) {
            ...
            common.getLocalIP(function (err, address) {
                ...
                build.build(project.paths.root, target, include,
                    function () {
                        next();
                    });
            });
            ...
            if (project.manifest.sdkVersion !== common.sdkVersion.toString()) {
                ...
                fs.writeFile(path.join(project.paths.root,
                    "manifest.json"), JSON.stringify(project.manifest,
                    null, '\t'));
            }
        });
    });
    ...
    function ServeSplash(req, res, next) {
        common.getProjectList(function (projects) {
            ...
            var img = project && project.manifest.splash && project.manifest
                .splash[splash];
            if (img) {
            ...
            if (!img || !fs.existsSync(img)) {
                ...
                if (project.manifest.splash && project.manifest.splash[
                    "universal"] && outSize) {
                    universalSplashPath = path.resolve(project.paths.root,
                        project.manifest.splash["universal"]);
                    logger.log("Splash image mapped from universal key (" +
                        universalSplashPath + ") ->", splash);
                } else {
                    universalSplashPath = build.common.paths.lib(
                        'defsplash.png');
                    logger.log(
                        "Splash image mapped from default placeholder (" +
                        universalSplashPath + ") ->", splash);
                }
                //run the splasher to generate a splash of the desired size
                var outImg = path.join(project.paths.root,
                    ".tempsplash.png");
                build.jvmtools.exec('splasher', ["-i", universalSplashPath,
                    "-o", outImg, "-resize", outSize, "-rotate", "auto"
                ], function (splasher) {
                    var formatter = new build.common.Formatter(
                        'splasher');
    ...
    app.get('/simulate/:debug/:shortName/:target/native.js', function (req, res,
        next) {
        common.getProjectList(function (projects) {
            var project = projects[req.params.shortName.toLowerCase()];
            var target = req.params.target;
            ...
            var dm = new Buffer(JSON.stringify({
                "name": "connect",
                "addr": req.ip
            }));
            var s = new dgram.createSocket("udp4");
            s.send(dm, 0, dm.length, 9320, '127.0.0.1', function (err,
                bytes) {
                s.close();
            });
        });
            ...
            try {
                ...
                if (fs.existsSync(routesJS)) {
                    ...
                    if (routes.initApp) {
                        ...
                        app.use('/simulate/addons/' + addonName + '/',
                            addonApp);
                    }
                }
                var staticFiles = addon.getPath('simulator', 'static');
                if (fs.existsSync(staticFiles)) {
                    app.use('/simulate/addons/' + addonName + '/static/',
                        etag.static(staticFiles));
                }
            } catch (e) {
                logger.error("simulator addon", addonName,
                    "failed to initialize", e);
            }
        }
    });
    app.get('/simulate/addons/', function (req, res) {
        res.json(simulatorAddons.map(function (addon) {
            return addon.getName();
        }));
    });
    app.get('/simulate/addons/:addon/index.js', function (req, res) {
        var name = req.params.addon;
        var addonPath = common.paths.addons(name, 'simulator', 'static',
            'index.js');
        if (fs.existsSync(addonPath)) {
            var JsioCompiler = require(common.paths.root('src', 'build',
                'compile')).JsioCompiler;
            ...
            compiler.inferOptsFromEnv('browser').compile(path, function (
                err, code) {
                res.send(";(function () { var jsio = GLOBAL.jsio; " +
                    code + "; exports = jsio('import " + path +
                    "')})();");
            });
        }
    });
