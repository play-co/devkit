...
exports = Class(function () {
    ...
    this.onNewTabClick = function () {
        this.newTab();
    };
    this.removeTab = function (el) {
        for (var i = 0;
            var len = el._buttons.length; i < len; ++i) {
    ...
    this.newTab = function (url, text, indent, noActivate) {
        ...
        el.reload = el.createButton('R', function () {
            el.status('loading...');
            frame.contentWindow.location = reloadURL(frame.contentWindow
                .location);
        });
        this.onResize();
        return el;
