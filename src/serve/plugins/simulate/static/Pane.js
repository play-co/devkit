...
exports = Class(sdkPlugin.SDKPlugin, function (supr) {
    ...
    this.onBeforeShow = function () {
        if (this._project == this._lastProject) {
            return;
        }
        ...
        if (!this._project) {
            frame.src =
                "javascript:document.body.innerHTML='Select a project'";
        } else {
            frame.src = '/simulate/' + this._project.appId +
                '/#hasServer=0&clients=[{"displayName":"woot","device":"iphone","rotation":1,"entry":"intro"}]';
        }
    };
