...
var Chrome = exports = Class(squill.Widget, function (supr) {
    ...
    this.init = function (opts) {
        ...
        this._mover = new squill.Drag().subscribe('DragStart', this,
            'onDragStart').subscribe('Drag', this, 'onDrag').subscribe(
            'DragStop', this, 'onDragStop');
        this._resizer = new squill.Drag().subscribe('DragStart', this,
            'onResizeStart').subscribe('Drag', this, 'onResize').subscribe(
            'DragStop', this, 'onResizeStop');
        ...
        window.addEventListener('keydown', bind(this, this._rebuildKeyListener),
            true);
    ...
    this.getContainer = function () {
        return this._frameWrapper || supr(this, 'getContainer');
    };
    ...
    this.sendEvent = function (evt) {
        ...
        case 'ROTATE':
            ...
            break;
        case 'SCREENSHOT':
            this._conn.sendRequest('SCREENSHOT', {}, null, function (err,
                res) {
                var canvas = res.canvasImg;
                var win = window.open('', '', 'width=' + (res.width + 2) +
                    ',height=' + (res.height + 2));
                ...
                doc.open();
                doc.write('<html><title>Screenshot ' + date + ' ' +
                    time + '</title></head>' +
                    '<body style="margin:0px;padding:0px;background-color:#000;">' +
                    '<img src="' + canvas + '">' + '</body></html>');
                doc.close();
            });
            break;
        case 'DRAG':
    ...
    this.getLaunchURL = function (params) {
        ...
        if (this._appId) {
            ...
            query.appId = this._appId = window.location.pathname.match(
                /simulate\/(.*)\//)[1];
        }
        ...
        var reservedKeys = ['debug', 'displayName', 'rotation', 'mute',
            'appID', 'test', 'i'
        ];
        hashKeys.forEach(function (hashKey) {
            if (reservedKeys.indexOf(hashKey) !== -1) {
                return;
            }
        ...
        var r = new std.uri('/simulate/' + (this._debug ? 'debug' :
                'release') + '/' + this._appId + '/' + this._params.target +
            '/').addQuery(query).addHash(hash).setProtocol("http").setHost(
            hostname).setPort(this._port);
        return r;
    };
    ...
    this.rotate = function () {
        if (this._canRotate) {
            ...
            this._el.style.WebkitTransform = 'rotate(' + (this._rotation %
                2 ? 90 : -90) + 'deg)';
            var onRotate = bind(this, function () {
                this._el.removeEventListener("webkitTransitionEnd",
                    onRotate);
    ...
    this.update = function () {
        ...
        this._canRotate = 'canRotate' in params ? !! params.canRotate :
            true;
        this._isDragEnabled = 'canDrag' in params ? !! params.canDrag :
            true;
        if (!params.canRotate) {
            this._rotation = 0;
        }
        ...
        if (isArray(params.screenSize)) {
            screenSize = params.screenSize[this._rotation % params.screenSize
                .length];
        } else {
        ...
        case 'facebook':
            ...
            if (!this._facebookBar) {
                ...
                bar.innerHTML =
                    "<div style='position:absolute;top:0px;left:0px;width:636px;height:44px;background:url(images/facebook-header-left.png)'></div>" +
        ...
        if (props && props.img) {
            ...
            if (bgWidth != this._background.width || bgHeight != this._background
                .height) {
    ...
    this.updateBackground = function (url) {
        ...
        if (this._backgroundProps.isRotated || this._rotation % 2 == 0) {
            ctx.drawImage(this._backgroundImg, 0, 0, ctx.canvas.width, ctx.canvas
                .height);
        } else {
            ...
            case 1:
                ...
                ctx.drawImage(this._backgroundImg, 0, 0, ctx.canvas.height,
                    ctx.canvas.width);
                break;
            }
        }
    ...
    this.onResizeStart = this.onDragStart = function () {
        this.setTransitionsEnabled(false);
    };
    ...
    this.onResizeStop = this.onDragStop = function () {
        this.setTransitionsEnabled(true);
    };
    this.onViewportChange = function () {
         ...
         */
        var x = Math.round(Math.max(0, (width - this._width) / 2)) + this._offsetX -
            this._backgroundProps.offsetX * this._scale;
        var y = Math.round(Math.max(0, (height - this._height) / 2)) + this
            ._offsetY - this._backgroundProps.offsetY * this._scale;
        $.style(this._frameWrapper, {
            marginTop: this._backgroundProps.offsetY * this._scale +
                'px',
            marginLeft: this._backgroundProps.offsetX * this._scale +
                'px'
        });
...
exports.buildChromeFromURI = function (uri) {
    ...
    return new Chrome({
        rotation: parseInt(uri.hash('rotation') || uri.query('rotation')) ||
            0,
        resolution: resolutions.get(uri.hash('device') || uri.query(
            'device'))
    });
};
