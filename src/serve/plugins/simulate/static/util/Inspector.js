...
var DetailsWidget = Class(Widget, function (supr) {
        ...
        this._children.forEach(function (child) {
            if (child instanceof PropRow) {
                child.props.forEach(function (prop) {
                    ...
                    this[prop.id].subscribe('ValueChange', this,
                        '_onValueChange', prop.id);
                }, this);
            }
        }, this);
    };
    ...
    this.imageView = function (uid, node) {
        ...
        this._inspector.getViewProps(uid, bind(this, function (err, res) {
            if (err) return;
            // show the drop area
            this.dropArea.style.display = res.isImageView ? "block" :
                "none";
    ...
    this.init = function () {
        ...
        this.dropArea.addEventListener("drop", bind(this, this.dropImage),
            false);
        this.saveImg.addEventListener("click", bind(this, this.saveImage),
            false);
    };
    this.dropImage = function (e) {
        ...
        reader.onload = bind(this, function (evt) {
            ...
            this._inspector.setImage(currentNode.currentImage.uid, evt.target
                .result);
...
exports = Class(Widget, function (supr) {
    ...
    this.setConn = function (conn) {
        ...
        conn.onEvent.subscribe('POLL_VIEW_POSITION', this, function (evt) {
            if (evt.args.uid === this._highlightUId) this._highlightPos =
                evt.args;
            if (evt.args.uid === this._selectedView) this._selectedPos =
                evt.args;
        });
        if (this._isShowing) {
            this.startDebugging();
    ...
    this.getViewProps = function (uid, cb) {
        if (this._conn && this._conn.isConnected()) {
            return this._conn.sendRequest('GET_VIEW_PROPS', {
                uid: uid
            }, null, cb);
        }
    };
    this.setViewProp = function (uid, key, value, cb) {
        if (this._conn && this._conn.isConnected()) {
            return this._conn.sendRequest('SET_VIEW_PROP', {
                uid: uid,
                key: key,
                value: value
            }, null, cb);
    ...
    this.setImage = function (uid, data, cb) {
        if (this._conn && this._conn.isConnected()) {
            cb = cb || function () {};
            this._conn.sendRequest('REPLACE_IMAGE', {
                uid: uid,
                imgData: data
            }, null, cb)
        }
    ...
    this.startDebugging = function () {
        ...
        this._conn.sendRequest('GET_ROOT_UID', null, null, bind(this,
            function (err, res) {
    ...
    this.startLocalDebugging = function () {
        ...
        this._unbindMouseout = bind(frame, 'removeEventListener',
    ...
    this.startRemoteDebugging = function (deviceId) {
        ...
        util.ajax.post({
            ...
            conn.onEvent.subscribeOnce('APP_READY', this, function (
                evt) {
    ...
    this.updateTrace = function () {
        ...
        for (var i = 0;
            var uid; uid = this._trace.list[i]; ++i) {
            var node = this._nodeIndex[uid];
            if (node) {
    ...
    this.close = function () {
        ...
        this._conn.sendEvent('SET_SELECTED', {
            uid: null
        });
        this._conn.sendEvent('SET_HIGHLIGHT', {
            uid: null
        });
        this.stopTimer();
        this._simulator.onViewportChange();
    ...
    this._disconnectMouseEvents = function () {
        if (this._conn && this._conn.isConnected()) {
            this._conn.sendEvent('POLL_VIEW_POSITION', {
                uid: null
            });
            this._conn.sendRequest('REMOVE_MOUSE_EVT');
        }
    };
    this._addNode = function (id, node) {
        this._nodeIndex[id] = node;
    };
    this._removeNode = function (id) {
        delete this._nodeIndex[id];
    };
});
var ViewNode = Class(Widget, function (supr) {
    ...
    this.init = function (opts) {
        ...
        this._subs = [];
        this._nodes = [];
        this._getChildren = opts.getChildren === undefined ? true : !! opts
            .getChildren;
        supr(this, 'init', arguments);
    };
    this.getViewUId = function () {
        return this._viewUId;
    };
    this.buildWidget = function (el) {
        var uid = this._viewUId;
        $.style(this._label, {
            paddingLeft: 10 + this._indent * 15 + 'px'
        });
        this._isToggled = false;
        this.refresh();
        this._subs.push($.onEvent(this._label, 'click', this, 'toggle'), $.onEvent(
            this._label, 'click', this, 'select'), $.onEvent(this._label,
            'mouseover', this._inspector, 'highlightView', uid), $.onEvent(
            this._label, 'mouseout', this._inspector, 'highlightView',
            null));
    ...
    this.destroy = function () {
        for (var i = 0;
            var unsubscribe; unsubscribe = this._subs[i]; ++i) {
            unsubscribe();
        }
        for (var i = 0;
            var node; node = this._nodes[i]; ++i) {
            node.destroy();
        }
        $.remove(this._el);
    ...
    this.refresh = function () {
        ...
        if (!this.sendGetView) {
            this.sendGetView = _.throttle(bind(this, function () {
                if (!conn || !conn.isConnected() || !this._getChildren || !
                    this._viewUId) {
                    return;
                }
                conn.sendRequest('GET_VIEW', {
                    uid: this._viewUId
                }, null, bind(this, function (err, res) {
                    ...
                    this._hasChildren = view.subviewIds &&
                        (view.subviewIds.length > 0);
                    ...
                    if (this._isToggled) {
                        ...
                        var uids = {};
                        for (var i = 0;
                            var n; n = this._nodes[i]; ++i) {
                            uids[n.getViewUId()] = n;
                        }
                        // refresh the list of nodes, reusing existing nodes
                        this._nodes = [];
                        for (var i = 0;
                            var uid; uid = view.subviewIds[
                                i]; ++i) {
                            if (uid in uids) {
                                // reuse and reorder
                                ...
                                if (i > 0 && this._nodes[i]
                                    ._el.previousSibling !=
                                    this._nodes[i - 1]._el) {
                                    this._childNodes.insertBefore(
                                        this._nodes[i]._el,
                                        this._nodes[i - 1]._el
                                        .nextSibling);
                                }
    ...
    this.updateToggleText = function () {
        var text = !this._hasChildren ? '\u25CF' : this._isToggled ?
            '\u25BC' : '\u25B6';
    ...
    this.toggle = function () {
        ...
        this.updateToggleText();
        $.style(this._childNodes, {
            display: this._isToggled ? 'block' : 'none'
        });
    };
});
