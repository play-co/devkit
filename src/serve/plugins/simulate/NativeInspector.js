...
var Server = Class(net.interfaces.Server, function () {
    ...
    this.setupRemoteConn = function (conn, args) {
        console.log('SETUP REMOTE CONN', args.device.globalId, args.device.info
            .model)
    ...
    this.proxyConnections = function () {
        ...
        this.unproxyConnections();
        this._inspectorConn.onRequest.subscribe('__any', this, function (
            name, req) {
            this._activeRemoteConn.sendRequest(name, req.args, null,
                function (err, res) {
                    req.respond(res);
                })
        });
        this._activeRemoteConn.onRequest.subscribe('__any', this, function (
            name, req) {
            this._inspectorConn.sendRequest(name, req.args, null,
                function (err, res) {
                    req.respond(res);
                })
        });
        this._inspectorConn.onEvent.subscribe('__any', this, function (name,
            evt) {
            this._activeRemoteConn.sendEvent(name, evt.args);
        });
        this._activeRemoteConn.onEvent.subscribe('__any', this, function (
            name, evt) {
