...
function serveProject(project) {
    ...
    _app.get(new RegExp("^/projects/" + id + "/files/(.*/|)$"), function (req,
        res) {
        ...
        if (fs.existsSync(dir)) {
            ...
            res.json({
                error: 'No folder by that name exists.',
                files: []
            }, 404);
        }
    });
    ...
    app.post("/projects/manifest/save/:shortName", function (req, res) {
        projectManager.getProjects(function (projects) {
            var project = projects.filter(function (project) {
                return project.manifest.shortName == req.params.shortName ||
                    project.manifest.appId == req.params.shortName;
            })[0];
            if (!project) {
                res.json({
                    ok: false,
                    error: 'Manifest for project not found'
                }, 404);
                return;
            }
