...
var ProjectCell = Class(squill.Cell, function () {
    ...
    this.render = function () {
        ...
        var hasGloss = project.manifest.ios && project.manifest.ios.icons &&
            project.manifest.ios.icons.renderGloss;
        ...
        var label = project.manifest && (project.manifest.title || project.manifest
            .name) || '<unknown>';
...
var ExampleCell = Class(squill.Cell, function () {
    ...
    this.render = function () {
        var project = this._data;
        this.name.setLabel(project.manifest && (project.manifest.title ||
...
exports = Class(sdkPlugin.SDKPlugin, function (supr) {
    ...
    this._filteredList = function (type) {
        return this._overview.getProjects().getFilteredDataSource(function (
            item) {
            return item.manifest.group && (item.manifest.group.toLowerCase() ===
                type.toLowerCase());
        });
    };
        ...
        for (var i = 0; i < panes.length; i++) {
            ...
            if (group) {
                var panel = this._children[0].newPane({
                    className: 'mainPanel',
                    title: group.toUpperCase()[0] + group.substr(-group
                        .length + 1),
                    children: [{
                        id: 'exampleList',
                        className: 'darkPanel',
    ...
    this.refresh = function () {
        if (!this.projectList || !this.projectList.getDataSource() || !this
            ._project) {
            return;
        }
    ...
    this.setOverview = function () {
        ...
        this.setDataSource(this._overview.getProjects().getFilteredDataSource(
            function (item) {
    ...
    this.buildWidget = function (el) {
        ...
        window.addEventListener('resize', bind(this.projectList,
            'needsRender'), false);
