...
function serveProject(project) {
    ...
    if (_trans[id] || !fs.existsSync(langPath)) {
        return false;
    }
    _trans[id] = {};
    ...
    if (shouldServe) {
        fs.writeFileSync(path.join(langPath, 'all.json'), JSON.stringify(_trans[
            id]));
...
exports.load = function (app) {
    ...
    wrench.LineReader.prototype.getNextLine = function () {
        ...
        var result = this.buffer.substring(0, lineEnd != -1 ? lineEnd : this.buffer
            .length);
