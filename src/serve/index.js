...
var STATIC_SIMULATE_DIR = path.join('src', 'serve', 'plugins', 'simulate',
    'static');
/**
 * Arguments.
 */
var argv = require('optimist').alias('port', 'p').describe('port',
    'Port on which to run the basil server.').
default ('port', 9200).alias('production').describe('production',
    'hit the production servers instead of the staging servers').
default (false).argv;
/**
 * Server API.
 */
...
function serveAPI(app) {
    ...
    app.get('/compiled/*', function (req, res) {
        ...
        compiler.compile('import ' + [req.params[0], 'preprocessors.import',
            'preprocessors.cls'
        ].join(','), function (err, src) {
            if (err) {
                res.send(err);
            } else {
                res.send(src);
    ...
    app.stack.unshift({
        route: '/.syntax',
        handle: express.urlencoded()
    });
    app.post('/.syntax', function (req, res) {
        var jvmtools = require('../build/jvmtools');
        jvmtools.exec("closure", ["--compilation_level", "WHITESPACE_ONLY",
            "--jscomp_off", "internetExplorerChecks", "--warning_level",
            "VERBOSE", "--js", "-"
            ...
            compiler.on("end", function (data) {
                ...
                errors.forEach(function (block) {
                    ...
                    var chr = null;
                    while (true) {
                        for (var j = 0; j < block.code.length; ++
                            j) {
                    ...
                    if (prefix.length) {
                        block.code = block.code.map(function (
                            line) {
...
function serveFrontend(app) {
    ...
    fs.readdirSync(path.join(__dirname, 'plugins')).forEach(function (name) {
        // Read the manifest.
        try {
            var manifest = path.join(__dirname, 'plugins', name,
                'manifest.json');
        ...
        // Serve static files.
        app.use(root + 'plugins/' + name, express.static(path.join(
            __dirname, 'plugins', name, 'static')));
...
function launchServer() {
    ...
    try {
        ...
        app.use(stylus.middleware({
            ...
            compile: function (str, path) {
                return stylus(str).set('paths', [common.paths.root(
                    "src/serve/plugins/")]).set('filename', path).set(
                    'compress', false).use(nib());
            }
        }));
    } catch (e) {
        logger.warn(e);
    ...
    common.getLocalIP(function (err, address) {
        address = address[0];
        build.jvmtools.exec('jmdns', ['-rs', 'basil', '_tealeaf._tcp',
            'local', basePort
        ], function (jmdns) {
...
exports.cli = function () {
    ...
    if (fs.existsSync('./manifest.json')) {
        logger.log(
            'Serving from project directory, attempting to automatically register..'
        );
