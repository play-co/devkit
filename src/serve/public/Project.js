...
exports = Class(function () {
    ...
    this.isAnimationPath = function (path) {
        ...
        while (i) {
            if (path === 'resources/animations/' + animations[--i].name +
                '.json') {
                return true;
            }
        }
        ...
        if (iconList) {
            ...
            for (var i = sizes.length - 1; i >= 0; i--) {
                if (sizes[i] <= targetSize) {
                    return '/projects/' + this.id + '/files/' + iconList[
                        sizes[i]];
                }
            }
            if (sizes.length) {
    ...
    this.getIcon = function (targetSize) {
        var path = this.manifest.icon ? ('/projects/' + this.id + '/files/' +
            this.manifest.icon) : null;
        if (!path) {
            path = this.tryIcons(this.manifest.android && this.manifest.android
                .icons, targetSize);
        }
        if (!path) {
            path = this.tryIcons(this.manifest.ios && this.manifest.ios.icons,
                targetSize);
        }
        if (!path) {
