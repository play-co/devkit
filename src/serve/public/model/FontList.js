...
var Font = Class(PubSub, function (supr) {
    this.init = function (opts) {
        ...
        this.src = '/projects/' + this._project.id +
            '/files/resources/fonts/' + opts.font.filename + '_0_0.png';
        this.colorImages = [];
    ...
    this._load = function (i, a, f) {
        ...
        image.src = '/projects/' + this._project.id +
            '/files/resources/fonts/' + this.font.filename + '_' + i + '_' +
            this[f] + '.png';
    };
    this.load = function () {
        var font = this.font;
        if (this.colorImageCount < this.font.count) {
            ...
            this._load(2, 'compositeOutlineImages',
                'compositeOutlineImageCount');
        } else {
...
exports = FontList = Class(PubSub, function (supr) {
    ...
    this._loadFont = function (font) {
        util.ajax.get({
            ...
            url: '/projects/' + this._project.id +
                '/files/resources/fonts/' + font.filename + '.json'
    ...
    this.rename = function (font, newName) {
        ...
        this._list.each(function (data) {
            found = found || (data.contextName === newName) && (data.font !==
                font);
        }, this);
        if (found) {
