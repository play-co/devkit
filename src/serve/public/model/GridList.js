...
var GridLayer = Class(function () {
    ...
    this.resize = function (width, height) {
        ...
        for (y = 0; y < height; y++) {
            line = [];
            for (x = 0; x < width; x++) {
                line[x] = this._fromData(this._data, x, y) || {
                    i: -1
                };
            }
            data[y] = line;
    ...
    this.setTile = function (x, y, tile, edge, mustResolve) {
        ...
        var imageGroups;
        var imageGroup;
        var properties1; var properties2;
        var width = this._width;
        var height = this._height;
        var find;
        var i; var j;
        if ((x >= 0) && (x < width) && (y >= 0) && (y < height)) {
            if (tileConfig.getApplyRules() && (edge !== undefined)) {
                ...
                if (!dest || (dest.i === undefined) || (dest.i === -1) || (
                    edge === 0)) {
                    this._data[y][x] = {
                        i: tile.i,
                        n: tile.n
                    ...
                    if ((properties1[dest.n] === undefined) || (properties2[
                        tile.n] === undefined)) {
                        if (mustResolve !== true) {
                            this._data[y][x] = tile;
                            result = true;
                        ...
                        if (imageGroups[find] !== undefined) {
                            ...
                            imageGroup = imageGroup[Math.round(Math.random() *
                                imageGroup.length) % imageGroup.length];
                            this._data[y][x] = {
                                i: imageGroup.i,
                                n: imageGroup.n
                            };
        ...
        if ((x1 === x2) && (y1 === y2)) {
            for (y = y1 - 1; y <= y2 + 1; y++) {
                for (x = x1 - 1; x <= x2 + 1; x++) {
                    ...
                    if (layer.setTile(x, y, tile, edge, !((x === x1) && (y ===
                        y1)))) {
                        result.tiles.push({
                            x: x,
                            y: y,
            ...
            for (y = y1; y <= y2; y++) {
                for (x = x1 - 1; x <= x2 + 1; x++) {
                    ...
                    edge |= (y === y2) ? (4 + 8) : 0;
                    undoTile = layer.getTile(x, y, true);
                    if (layer.setTile(x, y, tile, edge, (x === x1 - 1) || (
                        x === x2 + 1))) {
                        result.tiles.push({
                            x: x,
                            y: y,
            ...
            for (y = y1 - 1; y <= y2 + 1; y++) {
                for (x = x1; x <= x2; x++) {
                    ...
                    edge |= (y === y2 + 1) ? (4 + 8) : 0;
                    undoTile = layer.getTile(x, y, true);
                    if (layer.setTile(x, y, tile, edge, (y === y1 - 1) || (
                        y === y2 + 1))) {
                        result.tiles.push({
                            x: x,
                            y: y,
...
exports = GridList = Class(PubSub, function (supr) {
    ...
    this._loadGrid = function (grid) {
        util.ajax.get({
            url: 'code/' + this._project.id + '/resources/grids/' +
                grid.name + '.json',
            type: 'json'
        }, bind(this, function (err, response) {
            if (!err) {
    ...
    this.rename = function (grid, newName) {
        ...
        this._list.each(function (data) {
            found = found || (data.contextName === newName) && (data !==
                grid);
        }, this);
        if (found) {
