...
exports = Class(squill.TabbedPane, function (supr) {
    ...
    this.delegate = new Delegate(function (on) {
        on.simulateButton = function () {
            ...
            var simulateURL = "//" + targetHost + ":" + (+loc.port + 1) +
                "/simulate/" + proj.id +
                '/#simulators=[{"name":"Simulator_0","device":"iphone","rotation":' +
                rot + "}]";
    ...
    this.refresh = function () {
        ...
        this._overviewProjectIcon.style.backgroundImage = "url(" + (project
            .getIcon(512) || "/images/defaultIcon.png") + ")";
        this.simulateButton.setLabel(project.manifest.launchTitle ||
            "Simulate");
        if (project.manifest.ios && project.manifest.ios.icons && project.manifest
            .ios.icons.renderGloss) {
