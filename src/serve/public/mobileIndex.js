...
window.onload = function () {
    ...
    background.style.cssText =
        "position: fixed; z-index: -1; width: 100%; background: -webkit-linear-gradient(rgb(53, 82, 113), rgb(19, 29, 57)); top: 0px; bottom: -120px;";
    document.body.style.cssText =
        "height: 100%; font: 30px Helvetica; color: white; height: 100%";
    util.ajax.get({
        url: '/projects',
        type: 'json',
        data: {
        ...
        projects.forEach(function (appId) {
            createLink(appId);
        });
        ...
        function createHeader(text) {
            var node = document.body.appendChild(document.createElement(
                "h3"));
            node.innerText = text;
            node.style.cssText =
                'margin: 0px 0px 5px 0px; font-size: 16px; padding: 5px 0px 5px 10px; background: rgba(0, 0, 0, 0.2)';
        }

        function createLink(appId) {
            var link = document.createElement('a');
            link.setAttribute('style',
                'font-size: 12px; white-space: nowrap; text-overflow: ellipsis; color: white; width: 60px; overflow: hidden; text-decoration: none; display: inline-block; margin: 10px; text-align: center;'
            );
            link.innerHTML = "<br>" + appId;
            link.href = '/simulate/debug/' + appId + '/browser-mobile/';
            var icon = new Image();
            var src = getIcon(response[appId].manifest);
            icon.src = '/projects/' + appId + '/files/' + src;
            var dim = 60;
            icon.width = dim;
            icon.height = dim;
    ...
    function getIcon(manifest) {
        ...
        return icon || manifest.android && getIcon(manifest.android) ||
            manifest.ios && getIcon(manifest.ios) ||
