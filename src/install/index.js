...
var argv = require('optimist').usage(
    'Install packages from github. Defaults to the gameclosure account').boolean(
    'force').alias('force', 'f').
default ('force', false).boolean('ssh').describe('ssh',
    'Install addon through SSH').
default ('ssh', false);
exports.install = function (addon, version, cb) {
    ...
    common.track("BasilInstall", {
        "addon": addon
    });
    argv = argv.argv || argv; //lolz
    addonManager.install(addon, {
        version: version,
        force: argv.force,
        isSSH: argv.ssh
    }, function (err) {});
