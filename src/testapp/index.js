...
function exec(args, config, next) {
    var Optimist = require('optimist');
    var optimistParser = new Optimist(args).usage('Usage: ' + clc.greenBright(
            'basil testapp') + clc.yellowBright(' [target], ') + 'where ' + clc
        .yellowBright('[target]') + ' may be: ' + clc.yellowBright(targetNames.join(
            ', ')));
