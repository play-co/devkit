...
exports.register = function (directories, warn, cb) {
    ...
    async_forEach(directories, function (projectPath, next) {
        var f = ff(function () {
            ...
            if (err) {
                logger.error("Could not register project at",
                    projectPath);
                logger.error(err);
            }
            next();
...
exports.unregister = function (dirtounreg, next) {
    ...
    if (!fs.existsSync(path.join(dirtounreg, './manifest.json'))) {
        logger.log(
            "This isn't the root directory of a Game Closure SDK project!");
        return;
    } else {
        shortname = JSON.parse(fs.readFileSync(path.join(dirtounreg,
            './manifest.json'))).shortName;
    }
...
exports.clean = function (next) {
    ...
    if (cleanCount > 0) {
        common.config.set('projects', newReg, function () {
            console.log('Cleaned out', cleanCount,
                'projects that were invalid from the register.');
            next && next();
        });
    } else {
        next && next();
