 ...
 * Command line.
 */
var argv = require('optimist').usage('basil init [project-dir]').describe(
    'help', 'Show options.').alias('help', 'h').boolean('help').describe(
    'template', 'Pick a template to start building your game').alias('template',
    't').argv;
if (argv.help) {
...
var initProject = function (template, dest) {
    ...
    common.getProjectList(function (projects) {
        if (projects[shortName.toLowerCase()]) {
            console.log(
                "That project shortName is already taken.  Please rename your project or erase the old project and run `basil clean-register`"
            );
            return;
        }
        ...
        console.log('Created a new ' + template + ' project at ' +
            newLocation);
        common.track("BasilInit", {
            "template": template,
            "shortName": shortName
        });
