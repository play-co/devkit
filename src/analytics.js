...
function run() {
    if (!common.config.get("registerPrompt")) {
        var name;
        var f = ff(function () {
            logger.log(clc.yellow(
                "By default basil will send anonymous usage and crash reports"
            ));
            logger.log(clc.yellow("to help us improve the DevKit."));
            ask("Press " + clc.greenBright("enter") +
                " to continue.  Or type \"" + clc.redBright("no") +
                "\" to opt-out: ", f.slotPlain());
        }, function (optout) {
            ...
            logger.log("Please " + clc.greenBright("register") +
                " your product by providing us with your name and email address.  Leave these " +
                clc.redBright("blank") + " to opt-out:");
            ...
            if (!email.match(
                ...
                logger.log(
                    "Sorry you decided not to register.  If you change your mind, the " +
                    clc.yellowBright("form") + " is available here:");
                logger.log(
                    "https://docs.google.com/a/gameclosure.com/forms/d/1zYBFvzAlmZX4JWmWY1RkPxYlRJXUszLs4aWu2nt_kPs/viewform"
                );
                f.slotPlain()(false);
            } else {
            ...
            if (registered) {
                logger.log("Registration complete.  " + clc.greenBright(
                        "Thank you") +
                    " for using the Game Closure Devkit!");
            } else {
                logger.log(clc.greenBright("Thank you") +
                    " for using the Game Closure Devkit!");
            }
            process.exit(0);
        }).error(function (err) {
