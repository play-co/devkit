    ...
    if (!_repos) {
        ...
        Object.keys(repositories).forEach(function (repoName) {
            ...
            if (def.path) {
                ...
                if (!repo.exists()) {
                    throw new Error('repo ' + this.name +
                        ' has an invalid location: ' + this.location);
                } else {
            ...
            forEachRepo(function (repo) {
                ...
                var f2 = ff(function () {
                    ...
                    if (status.staged.length) {
                        summary += clc.greenBright(status.staged
                            .length + " files staged") +
                            " for commit";
                    }
                    if (status.modified.length) {
                        summary += (summary ? '; ' : '') + clc.redBright(
                    ...
                    if (commits.from.length) {
                        repo.log("");
                        repo.log(repo.getRemoteBranchStr(),
                            "has added", clc.redBright(commits.from
                                .length + " commits"),
                            "that you have not merged in yet");
                        repo.log("  ", "to see these commits:");
                        repo.log("  ", clc.whiteBright(
                            "basil contribute show " + repo
                            .name + " unmerged"));
                    }
                    if (commits.to.length) {
                        repo.log("");
                        repo.log("you have added", clc.redBright(
                                commits.to.length + " commits"),
                            "that are not merged upstream");
                        repo.log("  ", "to see these commits:");
                        repo.log("  ", "  ", clc.whiteBright(
                            "basil contribute show " + repo
                            .name + " commits"));
...
_commands.prerelease = Class(function () {
    ...
    this.init = function (argv) {
        var f = ff(function () {
            ...
            prompt(
                "warning: this will erase all uncommitted changes. type 'yes' to continue: ",
                f());
        }, function (response) {
            if (response == 'yes') {
                forEachRepo(function (repo) {
                    ...
                    repo.git([
                        ...
                        ['reset', '--hard', repo.getRemoteBranchStr()]
                    ], f());
                });
            } else {
...
_commands.release = Class(function () {
    ...
    this.init = function () {
        var argv = require("optimist").usage(
            'Usage: $0 -p --type [major|minor|patch] [--branch branch] [--channel channel]'
        ).boolean("production").string("author").
        default ("type", "patch").
        default ("branch", "master").
        default ("channel", "release").
        default ("test", false).argv;
        // tag submodules
        // add submodules
        ...
        var f = ff(function () {
            ...
            logger.log(
                "--- Writing new package.json and tagging repos with",
                tag);
            ...
            forEachRepo(function (repo) {
                ...
                if (repo.submodules) {
                    ...
                    // add all submodules
                    if (!argv.test) {
                        repo.git.apply(repo, ['add'].concat(paths).concat(
                            [f()]));
                    }
                }
            });
            ...
            function commitSM(repoName) {
                ...
                if (!repo.hasCommitted) {
                    ...
                    if (!repo.secondary) {
                        ...
                        if (!argv.test) {
                            repo.git('commit', '-m',
                                "Releasing version " + tag, f.waitPlain()
                            );
                        }
                    }
                }
            ...
            forEachRepo(function (repo) {
                if (repo.release && !repo.secondary) {
                    repo.log("pushing to", repo.remote, "HEAD:" +
                        repo.releaseBranch);
                    if (!argv.test) {
                        repo.git('push', '-f', repo.remote, "HEAD:" +
                            repo.releaseBranch, f());
                    }
                }
            }, this);
        }, function () {
            ...
            logger.log(
                "--- Pushing release remote to head release branch");
            // push the commits to release remote
            forEachRepo(function (repo) {
                if (repo.release && !repo.secondary) {
                    repo.log("pushing to", repo.release.remote,
                        "HEAD:" + repo.release.branch);
                    if (!argv.test) {
                        repo.git('push', '-f', repo.release.remote,
                            "HEAD:" + repo.release.branch, f());
                    }
                }
            });
        }, function () {
            ...
            forEachRepo(function (repo) {
                if (repo.release && !repo.secondary) {
                    ...
                    if (!argv.test) {
                        repo.git('push', repo.release.remote, tag,
                            f());
                    }
                }
            });
...
_commands.postrelease = Class(function () {
    ...
    this.init = function (argv, cb) {
        var f = ff(function () {
            ...
            forEachRepo(function (repo) {
                if (repo.release) {
                    repo.log("trying to merge ", repo.remote, repo.releaseBranch,
                        'into', repo.branch);
                    if (!argv || !argv.test) {
                        repo.git([
                            ['checkout', '-f', repo.getRemoteBranchStr()],
    ...
    this.init = function (argv) {
        ...
        case 'commits':
            repo.getCommitsSince(function (err, commits) {
                ...
                if (what == 'unmerged') {
                    list = commits.from;
                    logger = new common.Formatter(repo.name + ':' +
                        repo.remote);
                } else {
                    list = commits.to;
