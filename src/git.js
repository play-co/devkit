    ...
    var client = function () {
        ...
        if (Array.isArray(arguments[0])) {
            ...
            var next = function (code, out, err) {
                ...
                _err.push(err);
                if (code || !args[i]) {
                    return onResult(code, _out, _err);
                }
...
git.currentTag = function (gitDir, next) {
    var tagName = "Unknown";
    common.child('git', ["--git-dir", path.join(gitDir, ".git"), "describe",
        "--always", "--tag"
    ], {
...
git.commitPush = function (dir, message, next) {
    ...
    common.child('git', ['commit', '-m', message], opts, function (err, res) {
        common.child('git', ['push', 'origin', 'HEAD'], opts, function (
            err2, res2) {
...
git.branchIfNeeded = function (dir, branch, next) {
    ...
    common.child('git', ['branch'], opts, function (err, res) {
        ...
        common.child('git', ['checkout', '-b', branch], opts, function (
            err2, res2) {
            git.revert(dir, next);
        });
    });
...
git.getGithubHash = function (dir, next) {
    common.child('git', ['show', 'HEAD', '--pretty=oneline'], {
        cwd: dir,
        silent: true
    }, function (code, data) {
...
git.getGithubBranch = function (dir, next) {
    common.child('git', ['name-rev', '--name-only', 'HEAD'], {
        cwd: dir,
        silent: true
    }, function (code, data) {
        next(code, String(data).split(' ')[0]);
...
git.getGithubRepo = function (dir, next) {
    common.child('git', ['config', '--get', 'remote.origin.url'], {
        cwd: dir,
        silent: true
    }, function (code, data) {
        //change origin url to a link to the repo
        //regex is an mcav original
        next(code, String(data).replace(/^git@(.*?)\:([^\.]*?)(\.git)?$/,
            'http://$1\/$2'));
    });
};
