...
var argv = require('optimist').usage('basil update [options]').describe('tag',
    'Update to a specific tag.').alias('tag', 't').describe('channel',
    'Specify a channel (e.g "beta").').alias('channel', 'c').
default ('channel', 'release').describe('list-channel',
    'List possible channels.').alias('list-channel', 'l').
default ('list-channel', false).describe('list-version',
    'List possible channels.').alias('list-version', 'v').
default ('list-version', false).describe('reapply-changes',
    'Stash and reapply your changes.').alias('reapply-changes', 'r').
default ('reapply-changes', false).describe('help', 'Show options.').alias(
    'help', 'h').
default ('help', false).argv;
if (argv.help) {
    require('optimist').showHelp();
    process.exit(2);
}
if (argv['list-channel']) {
    common.child('git', ['tag', '-l'], {
...
if (argv['list-version']) {
    common.child('git', ['tag', '-l'], {
        ...
        if (argv.channel) data = data.filter(function (tag) {
            return tag && tag.channel == argv.channel
        });
        data.sort(Version.sorterDesc);
        for (var i = 0; i < data.length; ++i) {
            if (data[i]) console.log(data[i].toString());
        }
        process.exit();
    });
}
...
exports.update = function (tag, next) {
    ...
    var f = ff(this, function () {
        ...
        if (tag) {
            ...
            var wait = f.wait();
            common.child('git', ['tag', '-l'], defaultChildArgs, function (
                code, data) {
                var tagData = [];
        ...
        common.track("BasilUpdate", {
            "switchTag": tag.toString()
        });
        ...
        if (!err && data) {
            ...
            fs.writeFile(path.join(stashDir, 'stash_' + (+new Date())),
                String(data), f.wait());
        }
        console.log("Checking out version", tag.toString());
        common.child('git', ['checkout', '--force', tag.toString()],
            defaultChildArgs, f.wait());
    }, function () {
