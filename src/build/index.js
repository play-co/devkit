...
function createSDKSymlink(dir, next) {
    var from = common.paths.sdk();
    var to = path.join(dir, './sdk');
    // If a symlink exists, check that it points to the right location.
    try {
        if (!fs.lstatSync(to).isSymbolicLink()) {
            logger.error('File', to,
                'is not a symlink. Please remove this file and retry.');
            process.exit(2);
        }
        ...
        if (e.code == "ENOENT") {
            ...
            logger.warn('Unexpected exception trying to create a symlink at',
                to);
            console.error(e);
        }
    }
...
function build(dir, target, opts, cb) {
    ...
    var f = ff(function () {
        ...
        if (opts.outputDir) {
            ...
            opts.buildPath = path.join(project.paths.root, 'build',
                releasePath);
        }
        if (CREATE_SYMLINK) {
        ...
        if (exports.isTargetAvailable(supTarget)) {
            ...
            require(buildTargets[supTarget]).build(exports, project,
                subTarget, opts, f());
        } else {
        ...
        for (var addonName in buildAddons) {
            try {
                if (buildAddons[addonName].onAfterBuild) {
                    ...
                    buildAddons[addonName].onAfterBuild(exports, common,
                        project, buildRes.buildOpts, f());
                }
            } catch (e) {
                f.fail(e);
                logger.error("Error executing onAfterBuild for addon",
                    addonName, e);
            }
        }
    }, function () {
...
function exec(args, config, next) {
    var Optimist = require('optimist');
    var optimistParser = new Optimist(args).boolean('debug').
    default ('debug', config.template == "debug").describe('debug',
        '[native] builds a version with debugging support').boolean('compress')
        .
    default ('compress', config.template == "release").describe('compress',
        '[js] minifies JavaScript files').boolean('serverName').
    default ('serverName', 'local').describe('serverName',
        '[server] which server to build against, either a hostname or (local | staging | production)'
    ).string('outputDir').describe('outputDir', '[sdk] ').boolean('isSimulated')
        .
    default ('isSimulated', false).describe('isSimulated',
        '[sdk] internal use only').string('version').describe('version',
        'overrides manifest version if provided, specifies CONFIG.version').usage(
        'Usage: ' + clc.greenBright('basil build') + clc.yellowBright(
            ' [target]\n\n') + 'where ' + clc.yellowBright('[target]') +
        ' is one of:\n\n' + '\t' + _targetNames.join('\n\t'));
    ...
    if (!target || _targetNames.indexOf(target) == -1) {
        if (!target) {
            ...
            logger.error('"' + clc.yellowBright(target) +
