...
function getBuildOpts(project, baseOpts, targetOpts) {
    ...
    buildOpts.version = baseOpts.version || baseOpts.debug && 'debug' ||
...
function getConfigObject(project, opts, target) {
    ...
    try {
        var basilConfig = fs.readFileSync(common.paths.root(".version")).toString()
            .split(";");
    ...
    if (!manifest.splash || !Object.keys(manifest.splash).length) {
        ...
        config.splash = updateSplash(common.paths.root(
            "/src/init/templates/empty/"), opts.fullPath);
        manifest.splash = JSON.parse(JSON.stringify(config.splash));
    } else {
        config.splash = JSON.parse(JSON.stringify(manifest.splash));
    }
    if (manifest.android && (!manifest.android.icons || !Object.keys(manifest.android
        .icons).length)) {
        wrench.mkdirSyncRecursive(path.join(opts.fullPath, "resources/icons"));
        manifest.android.icons = updateIcons(common.paths.root(
            "/src/init/templates/empty/"), opts.fullPath, {
    ...
    if (manifest.ios && (!manifest.ios.icons || !Object.keys(manifest.ios.icons)
        .length)) {
        wrench.mkdirSyncRecursive(path.join(opts.fullPath, "resources/icons"));
        manifest.ios.icons = updateIcons(common.paths.root(
            "/src/init/templates/empty/"), opts.fullPath, {
...
function getJSConfig(project, opts, target, cb) {
    ...
    exports.getDefines(project, opts, function (defines) {
        ...
        cb(src.concat(["window.CONFIG=", JSON.stringify(getConfigObject(
                project, opts, target)), ";\n",
            ...
            if (!processedAddons[addonName]) {
                ...
                if (!fs.existsSync(path.join(addonsPath, addonName))) {
                    ...
                    addonManager.install(addonName, {}, function (err, res) {
                        if (!err) {
                            var deps = addonManager.getAddonDependencies(
                                addonName);
                            for (var d in deps) {
                                installAddon(deps[d]);
                            }
...
function getResources(project, buildOpts, cb) {
    ...
    var resourceDirectories = [{
        src: path.join(appDir, 'resources'),
        target: 'resources'
    }];
    ...
    var spritesheetsDirectory = path.join(appDir, buildOpts.localBuildPath,
    ...
    var f = ff(this, function () {
        ...
        Object.keys(addons).forEach(function (addonName) {
            ...
            try {
                if (addon && addon.hasBuildPlugin()) {
                    ...
                    var addonConfig = project.getAddonConfig();
                    var directories = buildAddon.getResourceDirectories &&
                        buildAddon.getResourceDirectories(addonConfig[
                            addonName]);
                    if (isArray(directories)) {
                        directories.forEach(function (directory) {
                            resourceDirectories.push({
                                src: directory.src,
                                target: path.join('addons',
                                    addonName, directory.target
                                )
                            });
                        });
                    }
                ...
                logger.error("Error initializing build addon",
                    addonName, e);
            }
        }, this);
        for (var addonName in buildAddons) {
            ...
            try {
                if (buildAddons[addonName].onBeforeBuild) {
                    buildAddons[addonName].onBeforeBuild(common, project,
                        buildOpts, onFinish);
                }
            } catch (e) {
                onFinish();
                logger.error("Error executing onBeforeBuild for addon",
                    addonName, e);
            }
        }
    }, function () {
        ...
        function spriteNextDirectory() {
            ...
            if (directory) {
                spriteDirectory(directory.src, directory.target, function (
                    err, res) {
                    if (!err) {
                        // merge results
                        resources.images = resources.images.concat(res.spritesheets);
                        if (resources.other) {
                            resources.other = resources.other.concat(
                                res.other);
                        }
                        Object.keys(res.imageMap).forEach(function (key) {
                            if (resources.imageMap[key]) {
                                logger.log(
                                    "WARNING: Resource aliasing.  A resource (",
                                    key,
                                    ") overwrote its key in map.json.  This is probably NOT what you want."
                                );
                            }
                            resources.imageMap[key] = res.imageMap[
                                key];
                        });
                    }
                    // next directory
        ...
        var relativeFontsheetSizeMap =
            "resources/fonts/fontsheetSizeMap.json";
        // absolute paths for font size output
        var fontsheetSizeMapFilename = path.join(appDir, buildOpts.localBuildPath,
            "resources", "fonts", "fontsheetSizeMap.json");
        ...
        if (!fontsDirExists) {
            ...
            fontsheetMap.create(fontsDir, "resources/fonts",
                fontsheetSizeMapFilename, f.wait());
        }
        spritesheetMap.create(resources.imageMap, spritesheetsDirectory,
            "spritesheetSizeMap", f());
    }, function (spritesheetMapPath) {
        resources.other.push(new Resource({
            fullPath: spritesheetMapPath,
            relative: path.relative(path.resolve(appDir, buildOpts.localBuildPath),
                spritesheetMapPath)
        }));
        var mapPath = path.join(spritesheetsDirectory, 'map.json');
        // write out the new image map
        fs.writeFile(mapPath, JSON.stringify(resources.imageMap), "utf8", f
            .wait());
        // add to resources
    ...
    function spriteDirectory(srcDir, targetDir, cb) {
        ...
        var f = ff(function () {
            ...
            var spriterOpts = ["--no-clean", // don't remove unused spritesheets, since we might be spriting multiple source directories into the same target
                "--cache-file", "spritercache-" + hash, "--scale", 1,
                "--dir", srcDir + "/", "--output", spritesheetsDirectory,
                "--target", buildOpts.target, "--binaries", common.paths.lib(),
                "--is-simulator", buildOpts.isSimulated,
                "--spriter-png-fallback", !! (buildOpts.argv && buildOpts.argv[
                    'spriter-png-fallback'])
            ];
            logger.log("spriter", spriterOpts.map(function (arg) {
            ...
            jvmtools.exec("spriter", spriterOpts, function (spriter) {
                ...
                spriter.on("end", function () {
                    onEnd(!out.length, out.join(""));
                });
            });
            ...
            var resources = {};
            resources.spritesheets = spriterOutput.sprites.map(function (
                filename) {
                var ext = path.extname(filename);
                return new Resource({
                    fullPath: path.resolve(spritesheetsDirectory,
                        filename),
                    relative: path.join(
                        relativeSpritesheetsDirectory, filename)
                });
            });
            var filteredPaths = [];
                ...
                if (path.basename(filename) === "metadata.json") {
                    try {
                        var filedata = fs.readFileSync(path.resolve(
                            srcDir, filename), "utf8");
                        var fileobj = JSON.parse(filedata);
                        if (fileobj.package === false) {
                            var filterPath = path.dirname(filename);
                            logger.log("Not packaging resources from",
                                filterPath);
                            filteredPaths.push(filterPath);
                        }
                    } catch (ex) {
                        logger.error("WARNING:", filename,
                            "format is not valid JSON so cannot parse it."
                        );
                    }
                }
                return new Resource({
            ...
            for (var ii = 0; ii < resources.other.length; ++ii) {
                ...
                if (path.basename(filename) === "metadata.json") {
                    logger.log("Did not package the metadata file",
                        filename);
            ...
            f(resources);
            var mapPath = path.resolve(spritesheetsDirectory, spriterOutput
                .map);
            ...
            Object.keys(rawMap).forEach(function (key) {
                if (rawMap[key].sheet) {
                    rawMap[key].sheet = useURISlashes(path.join(
                        relativeSpritesheetsDirectory, rawMap[key].sheet
                    ));
                }
                imageMap[useURISlashes(path.join(targetDir, key))] =
                    rawMap[key];
            });
...
function writeMetadata(opts, dir, json) {
    ...
    if (fs.existsSync(fontsDir) && fs.lstatSync(fontsDir).isDirectory() && !fs.existsSync(
        fontsMetadata)) {
...
function compileResources(project, buildOpts, initialImport, cb) {
    ...
    writeMetadata(buildOpts, "resources/icons",
        "{\"sprite\": false, \"package\": false}");
    writeMetadata(buildOpts, "resources/splash",
        "{\"sprite\": false, \"package\": false}");
    var f = ff(function () {
