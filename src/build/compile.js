 ...
 * Arguments.
 */
var argv = require('optimist').alias('verbose', 'v').describe('verbose',
    'Verbose mode.').boolean('verbose').argv;
/**
 * Default path for game js.io compilation.
 */
compile.JSIO_SDK_PATHS = ['sdk/jsio', 'sdk/gc/api/', 'sdk/', 'sdk/timestep/', ];
...
var BasilJsioInterface = Class(function () {
    ...
    this.compress = function (filename, src, opts, cb) {
        ...
        if (opts.compressorCachePath && filename) {
            try {
                var cacheFilename = (/^\.\//.test(filename) ? 'R-' +
                    filename.substring(2) : 'A-' + filename).replace(
                    /\.\.\//g, '--U--').replace(/\//g, '---');
                cachePath = path.join(opts.compressorCachePath,
                    cacheFilename);
                if (crypto) {
                ...
                if (fs.existsSync(cachePath)) {
                    fs.readFile(cachePath, 'utf8', function (err,
                        cachedContents) {
                        if (err) {
                            onCacheResult(err, src, true);
                        } else {
                            var i = cachedContents.indexOf('\n');
                            var cachedChecksum = cachedContents.substring(
                                0, i);
                            if (checksum == cachedChecksum) {
                                // Cache hit!
                                onCacheResult(null, cachedContents.substring(
                                    i + 1), false);
                            } else {
                                // File changed, need to compress
                                onCacheResult("cache mismatch", src,
                                    true);
                            }
                        }
                    }.bind(this));
            ...
            if (err) {
                ...
                try {
                    if (cachePath && filename) {
                        compressLog.log('Updating cache for', filename,
                            'at', cachePath);
...
exports.compress = function (filename, src, opts, cb) {
    var closureOpts = ['--compilation_level', 'SIMPLE_OPTIMIZATIONS', ];
    if (opts.noIE) {
...
compile.JsioCompiler = Class(function () {
    ...
    this.compile = function (entry, next) {
        ...
        var task = bind(this, function (next) {
            ...
            jsio_compile.compiler.start(['jsio_compile', this.opts.cwd ||
                '.', entry
            ], this.opts);
        });
